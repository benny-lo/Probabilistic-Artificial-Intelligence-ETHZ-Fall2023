...
class Model(object):
    ...
    __prediction_adjustment = 1.4852
    __samples = 7000
    ...
    def __init__(self):
        ...
        self.__model = GaussianProcessRegressor(kernel=kernel, normalize_y=True)
    ...
    def make_predictions(self, test_x_2D: np.ndarray, test_x_AREA: np.ndarray) -> typing.Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        (gp_mean, gp_std) = self.__model.predict(test_x_2D, return_std=True)
        
        # TODO: Use the GP posterior to form your predictions here
        predictions = gp_mean + test_x_AREA * self.__prediction_adjustment * gp_std
    ...
    def fitting_model(self, train_y: np.ndarray,train_x_2D: np.ndarray):
        ...
        n = len(train_y)
        assert n == train_x_2D.shape[0]

        undersample = self.rng.choice(np.arange(n), self.__samples, replace=False)
        self.__model.fit(train_x_2D[undersample], train_y[undersample])
