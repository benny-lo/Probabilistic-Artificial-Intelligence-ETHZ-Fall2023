...
def log_likelihood(data, dist): 
    return np.sum(np.log(dist.pdf(data)))
...
def log_posterior_probs(x):
    ...
    fun = np.vectorize(log_likelihood, excluded=['data'])

    log_li = fun(data=x, dist=HYPOTHESIS_SPACE)
    log_p = log_li + np.log(PRIOR_PROBS)

    normalizer = np.empty(len(HYPOTHESIS_SPACE))
    normalizer.fill(logsumexp(log_p))
