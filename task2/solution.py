...
class SWAGInference(object):
    ...
    def __init__(
        ...
        swag_epochs: int = 60, #30
        swag_learning_rate: float = 0.045,
        swag_update_freq: int = 1,
        deviation_matrix_max_rank: int = 30, #15
        bma_samples: int = 60, #30
    ):
        """
    ...
    def update_swag(self) -> None:
        ...
        for name, param in current_params.items():
            ...
            n_models = self.iteration / self.swag_update_freq
            self.theta_mean[name] = (n_models * self.theta_mean[name] + param) / (n_models + 1)
            self.theta_squared_mean[name] = (n_models * self.theta_squared_mean[name] + torch.mul(param, param)) / (n_models + 1)
        ...
        if self.inference_mode == InferenceMode.SWAG_FULL:
            ...
            dev = self._create_weight_copy()
            for name in dev: 
                dev[name] = current_params[name] - self.theta_mean[name]
            self.deviation_matrix.append(dev)
    ...
    def fit_swag(self, loader: torch.utils.data.DataLoader) -> None:
        ...
        pretrained_param = {name: param.detach() for name, param in self.network.named_parameters()}
        for name, param in pretrained_param.items(): 
            self.theta_mean[name] = param
            self.theta_squared_mean[name] = torch.mul(param, param)
        ...
        with tqdm.trange(self.swag_epochs, desc="Running gradient descent for SWA") as pbar:
            ...
            for epoch in pbar:
                ...
                self.iteration += 1
                if self.iteration % self.swag_update_freq == 0:
                    self.update_swag()                
    ...
    def predict_probabilities_swag(self, loader: torch.utils.data.DataLoader) -> torch.Tensor:
        ...
        bma_probabilities = 1.0 / len(per_model_sample_predictions) * sum(per_model_sample_predictions)
    ...
    def sample_parameters(self) -> None:
        ...
        for name, param in self.network.named_parameters():
            ...
            current_mean = self.theta_mean[name]
            current_std = torch.sqrt(
                torch.clamp(
                    self.theta_squared_mean[name] - torch.mul(self.theta_mean[name], self.theta_mean[name]), 
                    self.variance_clamp)
                    )
            ...
            # Diagonal part
            mult = 1.0
            if self.inference_mode == InferenceMode.SWAG_FULL: mult = math.sqrt(0.5)
            
            sampled_param = current_mean + mult * current_std * z_1

            # Full SWAG part
            if self.inference_mode == InferenceMode.SWAG_FULL:
                # TODO(2): Sample parameter values for full SWAG
                mult *= math.sqrt(1.0 / (self.deviation_matrix_max_rank - 1))
                z_1 = torch.randn(self.deviation_matrix_max_rank)
                for dev, coeff in zip(self.deviation_matrix, z_1): 
                    sampled_param += mult * coeff * dev[name]
    ...
    def _create_tensor_copy(self, tensor: torch.Tensor) -> torch.Tensor: 
